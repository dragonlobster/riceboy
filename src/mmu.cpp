...
uint8_t mmu::get_value_from_address(uint16_t address) const {
    ...
    else if (locate_section(address) == mmu::section::cartridge_header_area) {
        return this->cartridge_header_area[address - 0x0100];
    }

    else if (locate_section(address) == mmu::section::cartridge_rom_bank_0) {
    ...
    else if (locate_section(address) == mmu::section::character_ram) {
        return this->character_ram[address - 0x8000];
    }
    ...
    else if (locate_section(address) == mmu::section::cartridge_ram) {
        return this->cartridge_ram[address - 0xa000];
    }

    else if (locate_section(address) == mmu::section::internal_ram_bank_0) {
    ...
    else if (locate_section(address) == mmu::section::echo_ram) {
    ...
    else if (locate_section(address) == mmu::section::zero_page) {
        return this->zero_page[address - 0xff80];
    }

    else if (locate_section(address) == mmu::section::interrupt_enable_flag) {
...
void mmu::write_value_to_address(uint16_t address, uint8_t value) {
    ...
    // only before boot rom complete
    else if (locate_section(address) == mmu::section::unusuable_memory) {
        if (!boot_rom_complete) {
            this->unusable_memory[address - 0xfea0] = value;
        }
    }
        ...
        std::cout
            << "(write) memory not implemented: " //<< static_cast<unsigned
                                                  //int>(opcode)
            << "hex: 0x" << std::hex << static_cast<unsigned int>(address)
            << std::endl;
    }
