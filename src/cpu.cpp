...
void cpu::bit_b_r8(const registers &r8, uint8_t b) {
    ...
    auto get_bit_of_r8 = [=]() {
        ...
        this->W = (register_value >> b) & 1; // get the bit
...
void cpu::inc_or_dec_r8(const registers &r8, bool inc) {
...
void cpu::inc_or_dec_r16(const registers &r1, const registers &r2, bool inc, bool sp) {
...
void cpu::ld_rr_address(const registers &r1, const registers &r2, bool sp) {
    ...
    registers r1_test = r1;
    registers r2_test = r2;
...
void cpu::load_boot_rom() {
    ...
    if (file.is_open()) {
        ...
        std::vector<char> buffer(size); // prepare buffer with size = size of file
...
int cpu::handle_opcode(const uint8_t &opcode) {
    ...
    switch (opcode) {
        ...
        case 0x0e: {
            ld_r_imm8(registers::C);
            break;
        }

        case 0x3e: {
            ld_r_imm8(registers::A);
            break;
        }

        case 0xe2: {
            //ld_imm16_a(<#bool to_a#>)
            // TODO: implement LDH (C) A
            break;
        }

        case 0x0c: {
            inc_or_dec_r8(registers::C, true);
            break;
        }
        ...
        case 0xe0: {
            // TODO: implement LDH (n)
            break;
        }*/

        default:
            // print out the opcodes we didnt' implement
            std::cout << "not implemented: " << static_cast<unsigned int>(opcode) << " hex: " << std::hex << static_cast
...
int cpu::handle_cb_opcode(const uint8_t &cb_opcode) {
    switch (cb_opcode) {
        ...
        */
        ...
        default:
            // print out the opcodes we didnt' implement
            std::cout << "not implemented extended (cb): " << std::hex << static_cast<unsigned int>(cb_opcode) <<
                    std::endl;
        //system("pause");
            std::cin.get();
            return 0;
    ...
    if (ticks < 4) {
        ...
        switch (this->fetch_opcode) {
            case true: {
                ...
                const uint8_t opcode = this->_read_memory(this->PC); // get current opcode
